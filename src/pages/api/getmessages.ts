...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    let receiver_id = req.body.receiver_id as string;
    let page = req.body.page as string;

    let response = await axios.get(`http://127.0.0.1:3535/get_messages?receiver_id=${receiver_id}&page=${page}`, {
      headers: {
